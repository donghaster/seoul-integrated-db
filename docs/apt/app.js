...
(function () {
  ...
  /* 평형대 — 전용면적의 정수 부분이다. 반올림하면 안 된다.

     주택형 이름은 전용면적을 버린 값으로 붙는다. 84.91㎡는 '84타입'이지
     '85타입'이 아니다. 반올림하면 이름이 틀릴 뿐 아니라 한 주택형이 둘로
     쪼개진다 — 트리니원 100형은 100.16~100.69인데 반올림하면 100과 101로,
     112형(112.08~112.95)은 112와 113으로 갈라져 거래가 나뉘고 중위값도
     따로 잡혔다. 버리면 59·84·100·112·126으로 조합 자료의 주택형 번호와
     그대로 맞는다. */
  function areaBand(a) { return Math.floor(a); }
  ...
  /* 평형 이름은 늘 한 단지 안에서 쓰인다. 그 단지 전용률을 알면 그것으로
  ...
  /* 분양(공급)면적은 실거래 자료에 없다 — 국토교통부는 전용면적만 준다.
     ...
     단지별 실제값을 아는 곳은 data/supply.js에 적어 두고 그대로 쓴다. 그 표는
     건축물대장 전유공용면적부에서 호별 전유면적과 주거공용면적을 받아 더한
     것이라(tools/fetch_supply.py) 어림이 아니다. 아직 안 받은 단지는 아래
     평균 전용률로 어림하고, 어느 쪽인지 화면에도 밝힌다. */
  ...
  /* 이 단지의 전용률.
     ...
     평형별 표나 유사 단지 표의 평당가는 여러 평형에 걸친 중위값이라, 특정
     한 타입의 공급면적으로 환산할 수가 없다. 그럴 때는 그 단지의 전용률을
     쓴다 — 단지표에 있는 타입들의 전용/공급 비를 평균한 값이다.
     모르는 단지는 서울 평균으로 돌아간다. */
  var RATIO_MEMO = {};

  function ratioOf(gu, dg, name) {
    var key = gu + "|" + dg + "|" + name;
    if (RATIO_MEMO[key] != null) return RATIO_MEMO[key];
    var t = SUPPLY_TBL[key], sum = 0, n = 0;
    if (t) {
      for (var k in t) {
        if (k.charAt(0) === "_") continue;
        var ex = parseFloat(k), sp = t[k];
        if (ex > 0 && sp > 0) { sum += ex / sp; n++; }
      }
    }
    var r = n ? sum / n : (t && t._ratio ? t._ratio : SUPPLY_RATIO);
    RATIO_MEMO[key] = r;
    return r;
  }
  ...
  function renderIndexScript(sets, labels, isIdx) {
    ...
    sets.forEach(function (d) {
      ...
      if (!total) {
        out.push("<b>" + esc(d.label) + "</b>" + josa(d.label, "은", "는") + " 이 기간에 <b>거래 없음</b>.");
        return;
      }
      var v = usable(d);
      if (v.length < 2) {
      ...
      if (v.length >= 3) {
        ...
        if (lo && (hi - lo) / lo * 100 >= 15) {
          t += " 그런데 <b>쭉 한 방향으로 움직인 건 아님</b><br>\u2014 가장 높았던 " + unitEun + " " +
  ...
  function renderBriefScript(mo) {
    ...
    function trend(get, label) {
      ...
      if (!total) return "<b>" + label + "</b>" + josa(label, "은", "는") + " 이 기간에 <b>거래 없음</b>.";
      ...
      var t = "<b>" + label + "</b>는 " + moLabel(a2.m) + " 평당 " +
        ...
        word + ".";
      /* 신고가 덜 찬 달과 출렁임을 따로 끊으면 '아님. 그런데 … 아님.'으로
         같은 꼬리가 두 번 온다. 둘 다 붙는 경우는 한 문장으로 잇는다. */
      var pendTail = isPending(b2.m);
      if (pendTail) {
        t += " 다만 " + moLabel(b2.m) + "은 <b>신고가 아직 다 안 들어와</b> 확정된 숫자가 아니";
      }
      ...
      // 출렁임 문장이 안 붙었으면 위 '…아니'를 여기서 맺어 준다
    ...
    if (w.length >= 2) {
      ...
      var t3 = "<b>월세</b>는 중위 월세가 " + Math.round(wa.wolse.rent).toLocaleString() + "만원에서 " +
        ...
        (Math.abs(rentGap) < 1 ? "거의 그대로" : (rentGap > 0 ? "<b>" + rentGap.toFixed(0) + "% 상승</b>"
                                                              : "<b>" + Math.abs(rentGap).toFixed(0) + "% 하락</b>")) +
        ", 보증금은 " + eokman(wa.wolse.dep) + "에서 " + eokman(wb.wolse.dep) + ", " +
        "준전세 비중은 " + jrA + "%에서 " + jrB + "%.";
      if (jrB - jrA >= 8 && rentGap < 0) {
        t3 += " <b>보증금을 올리고 월세를 낮추는 쪽</b>으로 이동.<br>" +
              "위 가격지수 그래프의 월세선은 환산보증금 기준이라 <b>거꾸로 올라 보이지만, 월세가 오른 것은 아님</b>.";
