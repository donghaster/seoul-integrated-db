...
(function () {
  ...
  function drawFp(t) {
    ...
    if (night >= 25) lines.push("심야·새벽 비중이 <b>" + night + "%</b>로 높음. " +
      "다만 <b>00~06시는 자고 있는 주민이 함께 잡히는 칸</b>이라, 그 자체를 손님으로 보면 안 됨.");
  ...
  function slHtml(t) {
    ...
    return '<div class="mini-row">' +
      ...
      '<div class="read-guide" style="margin-top:16px"><h4>금집부쌤이 보는 매출</h4><ol>' +
        ...
        "<b>업종마다 편차가 큼</b> — 위 업종별 그래프를 함께 볼 것.</li>" +
        "<li>" + (weDay > mdwkDay ? "<b>주말 매출이 평일보다 높음.</b> 주말 장사가 되는 자리."
                                   : "<b>평일 매출이 주말보다 높음.</b> 주말에 쉬는 업종도 고려해 볼 만함.") + "</li>" +
        "<li><b>서울시가 카드 결제 등으로 추정한 값</b>. 실제 매출과 다를 수 있음 &mdash; " +
        "<b>매도인 장부와 반드시 대조</b>하고, 고객께도 추정치임을 밝힐 것.</li>" +
      "</ol></div>";
  }
  ...
  function indBrief(name, r, net, per, bPer, gap, t) {
    ...
    if (r.c >= 20) {
      lines.push(net > 0
        ...
        : net < 0
          ? "그 분기에 <b>" + comma(r.o) + "곳이 열고 " + comma(r.x) + "곳이 닫아 " + (-net) +
            "곳 줄어듦.</b> <b>빠져나가는 중 &mdash; 이유를 꼭 확인할 것.</b>"
          : "그 분기에 <b>연 곳과 닫은 곳이 같음.</b> 자리는 유지되는 중.");
    } else {
      lines.push("<b>점포가 " + comma(r.c) + "개뿐이라</b> 개업·폐업 숫자를 흐름으로 읽으면 안 됨.");
    }
    if (per && bPer) {
